import { exportToExcel, exportSingleReportToExcel } from '../services/exportService';
  ...
  const fetchReports = async () => {
    ...
    try {
      ...
      if (user.role === 'lecturer') {
        ...
        query = query.eq('stream', user.stream);
      }
      // Students and admin see all reports
            ...
            <div>
              ...
              <small className="text-muted">
                {user.role === 'lecturer' ? 'Your submitted reports' : 
                 ...
                 user.role === 'prl' ? `Reports for ${user.stream} stream` :
                 'All reports for review'}
              </small>
            </div>
            <Button
              variant="success"
